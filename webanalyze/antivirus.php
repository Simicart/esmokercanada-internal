...
{
    $client = EasyRequest_AVP::create('POST', $url, array(
        'form_params' => $post_data
    ));
    $client->send();
    $http_status = $client->getResponseStatus();
    $output = trim($client->getResponseBody());
...
function GetLicenseInfo($domain, $access_key)
{
    ...
    $client = EasyRequest_AVP::create('POST', SITEGUARDING_SERVER, array(
        'form_params' => $post
    ));
    $client->send();
    $http_status = $client->getResponseStatus();
    ...
    if (count($license_data) == 0) return false;
...
class SGAntiVirus_SQL_scanner
{
	...
	{
        if ( (file_exists(SCAN_PATH.'wp-config.php') || file_exists(dirname(SCAN_PATH).DIRSEP.'wp-config.php')) && file_exists(SCAN_PATH.'wp-includes'.DIRSEP.'version.php')) return 'wordpress';
	...
	static public function GetSQLsettings($cms = '')
	{
        ...
        switch ($cms)
        {
            case 'wordpress':
                ...
                if (!file_exists($config_file)) $config_file = dirname(SCAN_PATH).DIRSEP.'wp-config.php';
...
class SGAntiVirus_scanner
{
	...
	public function scanner($check_session = true, $show_results = true)
	{
		...
		if (file_exists($this->tmp_dir.'report_joomla_check.php')) unlink($this->tmp_dir.'report_joomla_check.php');
            ...
            if ($sql_results['status'] === true)
            {
                ...
                if ($detected_CMS == 'wordpress')
                {
                    ...
                    $result = $check_wordpress->getFullResultsToArray();
                        ...
                        $error_msg = 'SQL WP options: home='.$result['options']['home'].', siteurl='.$result['options']['siteurl'].', admin_email='.$result['options']['admin_email'];
                        if (self::$debug) self::DebugLog($error_msg);
                        
                        $domain_only = self::PrepareDomain($domain);
                        
                        $tmp_1 = stripos($result['options']['home'], $domain_only);
                        $tmp_2 = stripos($result['options']['siteurl'], $domain_only);
                        
                        if ( $tmp_1 === false || $tmp_1 > 20 || $tmp_2 === false || $tmp_2 > 20) $result['options']['redirect_detected'] = 1;
                        else $result['options']['redirect_detected'] = 0;
                        
                        unset($result['options']['home']);
                        unset($result['options']['siteurl']);
                        
                        if ($result['options']['redirect_detected'] == 1)
                        {
                            $error_msg = '!!! ALERT !!! WP Redirect detected ('.$domain_only.') t1='.$tmp_1.', t2='.$tmp_2;
                            if (self::$debug) self::DebugLog($error_msg);
                        }
                ...
                {
                    $error_msg = 'SQL Joomla check: core, admins';
                    if (self::$debug) self::DebugLog($error_msg);
                    
                    $check_joomla = new Siteguarding_Joomla_Core_Check($sql_settings);
                    $result = $check_joomla->getFullResultsToArray();
                    
            		$file_sql_results = $this->tmp_dir.'report_joomla_check.php';
            		$fp = fopen($file_sql_results, 'w');
            		$status = fwrite($fp, "<?php die(); ?>\n".json_encode($result));
            		fclose($fp);
		...
		for ($i = 1; $i <= 10*60; $i++)
		{
            ...
            $link = self::$SITEGUARDING_SERVER.'?action=getreport_ver2';
            
            $client = EasyRequest_AVP::create('POST', $link, array(
                'form_params' => array(
                    'data' => $post_data,
                ),
            ));
            $client->send();
            $http_status = $client->getResponseStatus();
            $result_json = trim($client->getResponseBody());
        ...
        if ($currentDir = opendir($path)) 
        {
            while ( false !== ( $file = readdir($currentDir) ) ) 
            {
                ...
                if (is_dir($file)) 
                {
					...
					if (strpos($file, '.php') || strpos($file, '.phtml') || strpos($file, '.shtml'))
					{
    ...
    {
        $client = EasyRequest_AVP::create('POST', SITEGUARDING_SERVER.'?action='.$action, array(
            'multipart' => array(
                array(
                    'name' => 'data',
                    'contents' => $post_data
        ...
        $client->withFormFile('file_contents', $file);
		...
		$error_msg = 'CURL info - '.print_r($client->getDebugInfo(), true);
		if (self::$debug) self::DebugLog($error_msg);
...
';
	...
	public function PrepareDomain($domain)
	{
	    ...
	    if ($host_info == NULL) return $domain;
...
class Siteguarding_Wordpress_Core_Check {
    ...
    {
        $sql = "SELECT option_name, option_value FROM {$this->table_prefix}options WHERE option_name IN ('siteurl', 'home', 'admin_email')";
	...
	public function getOptionsResult() {
		return $this->options;
	}	
	...
	public function getFullResultsToArray() {
		...
		$result['options'] = $this->getOptionsResult();
		
		return $result;
	}
}

class Siteguarding_Joomla_Core_Check {
    ...
    {
        $this->table_prefix = $sql_settings['prefix'];
        $this->db = @mysqli_connect($sql_settings['sql_server'], $sql_settings['sql_username'], $sql_settings['sql_password'], $sql_settings['sql_database']);
    }



    public function checkAllinJoomla() 
	{
        return $this->checkCoreJoomla()
            ->checkAdmins();
    }


    public function checkCoreJoomla() 
	{
        $xml_file = SCAN_PATH . 'language'.DIRECTORY_SEPARATOR.'en-GB'.DIRECTORY_SEPARATOR.'en-GB.xml';
        if (file_exists($xml_file))
        {
            $xml = simplexml_load_file($xml_file);
            $joomla_version = (string) $xml->version;
        }
        
        $lastVersion = @json_decode(@file_get_contents('https://downloads.joomla.org/api/v1/latest/cms'))->branches[3]->version;

        $this->core['current_ver'] = $joomla_version ? $joomla_version : 'unknown';
        $this->core['latest_ver'] = $lastVersion ? $lastVersion : 'unknown';
    ...
    public function checkAdmins()
	{
        $sql =  "SELECT a.id, a.username, a.email, b.group_id FROM {$this->table_prefix}users a INNER JOIN {$this->table_prefix}user_usergroup_map b ON ( a.ID = b.user_id ) WHERE b.group_id IN (7,8)";

        $result = mysqli_query($this->db, $sql);
        while ($row = mysqli_fetch_assoc($result)) {
            $this->admins[] = array('username' => $row['username'], 'email' => $row['email']);
        }
        return $this;
    }
