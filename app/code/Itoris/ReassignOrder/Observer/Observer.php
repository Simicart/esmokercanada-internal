...
class Observer implements ObserverInterface
{
    ...
    {
        $helper = $helper = Mage::helper('itoris_reassignorder');
        if ($helper->getSettings()->getEnabled()) {
            $block = $observer->getEvent()->getBlock();
            if ($block instanceof Mage_Adminhtml_Block_Widget_Grid_Massaction && $block->getRequest()->getControllerName() == 'sales_order') {
                $block->addItem('assign_order', array(
                    'label'        => Mage::helper('customer')->__('Assign to Customer'),
                    'url'          => Mage::getUrl('adminhtml/itorisreassignorder_index/massReassign'),
                ));
            }
        }
    }
