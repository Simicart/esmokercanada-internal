...
class Tracking extends \Mageside\CanadaPostShipping\Model\Service\AbstractService
{
    ...
    public function processTrackingDetails($trackInfo)
    {
        ...
        foreach ($trackInfo as $key => $event) {
            if ($event->{'event-description'} == 'Delivered') {
                $result['deliverydate'] = $event->{'event-date'};
                $result['deliverytime'] = $event->{'event-time'};
            } elseif ($key == 0) {
                $result['shipped_date'] = $this->getDeliveryDateTime($event);
    ...
    {
        return $this->formatDate($trackInfo->{'event-date'})
            . ' '
            . $this->formatTime($trackInfo->{'event-time'});
    }
}
