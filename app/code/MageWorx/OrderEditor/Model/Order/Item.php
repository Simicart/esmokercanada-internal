    ...
    protected function editCustomOptions($productOptions = [])
    {
        ...
        foreach ($productCustomOptions as $option) {
            ...
            if ($optionsFromRequest !== null && isset($optionsFromRequest[$option->getId()])) {
                 ...
                 */
                if (stripos($optionsFromRequest[$option->getId()], ',') !== false) {
                    $optionsFromRequest[$option->getId()] = explode(',', $optionsFromRequest[$option->getId()]);
                }
                if (is_array($optionsFromRequest[$option->getId()])) {
                    foreach ($optionsFromRequest[$option->getId()] as $aValue) {
                        $valueTitles[] = $option->getValueById($aValue)->getTitle();
                    }
                    $valueTitle  = implode(',', $valueTitles);
                    $valueDirect = implode(',', $optionsFromRequest[$option->getId()]);
                } else {
                    $value       = $option->getValueById($optionsFromRequest[$option->getId()]);
