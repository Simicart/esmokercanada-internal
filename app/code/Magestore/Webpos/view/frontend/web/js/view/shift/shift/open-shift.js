...
define(
    ...
    function ($, ko, Component, shift, currentShift, priceHelper, shiftHelper, notification, onlineResource, datetimeHelper, Event, PosManagement, CashCounting, Cookies) {
        "use strict";
        ...
        return Component.extend({
                ...
                Event.observer('show_open_session_popup_after', function (event, eventData) {
                    ...
                    checkListPos.always(function(response){
                        if(response && (response.total_count == 0)){
                            self.showError(true);
                            self.errorMessage('There are 0 POS available');
                        }else{
            ...
            OpenNewShift: function () {
                ...
                self.loading(true);
                var checkAssign = $.Deferred();
                PosManagement.assign('', '', checkAssign);
                checkAssign.done(function(response){
                    if(response == true){
            ...
            checkSessionOpen: function(){
                var self = this;
                if(!PosManagement.validate() && !Cookies.get('check_login')){
