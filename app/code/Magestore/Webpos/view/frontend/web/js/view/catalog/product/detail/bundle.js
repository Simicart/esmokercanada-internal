...
define(
    ...
    function ($,ko, detailPopup, priceHelper) {
        ...
        return detailPopup.extend({
            ...
            getSelectionTitlePrice: function (price,itemData) {
                if(itemData){
                    if(itemData.is_default == 1){
                        if(itemData.selection_can_change_qty == 0){
                            if ($("#bundle-option-" + itemData.option_id + "-qty-input").length > 0) {
                                $("#bundle-option-" + itemData.option_id + "-qty-input")[0].setAttribute('disabled', true);
                            }
                        }else{
                            if ($("#bundle-option-" + itemData.option_id + "-qty-input").length > 0) {
                                $("#bundle-option-" + itemData.option_id + "-qty-input")[0].removeAttribute('disabled');
                            }
                        }
                    }
                }
            ...
            updatePriceSelect: function (bundleOptions) {
                var self = this;
                if(!bundleOptions){
                        ...
                        $.each(itemsData, function (indexItem, valueItem) {
                            if (($("#bundle-option-" + valueItem.option_id).length > 0)
                                ...
                                if (valueItem.selection_id == selectId) {
                                    ...
                                    $("#bundle-option-" + valueItem.option_id + "-qty-input").val(parseInt(qty));
                ...
                self.updatePrice(bundleOptions);
            },
                    ...
                    if (value.type == 'radio') {
                        ...
                        $.each(itemsData, function (indexItem, valueItem) {
                            if (($("#bundle-option-" + valueItem.option_id + "-" + valueItem.selection_id).length > 0)
                                ...
                                if(valueItem.selection_can_change_qty == 0){
                                    qty = parseFloat(valueItem.selection_qty);
                                    if ($("#bundle-option-" + valueItem.option_id + "-qty-input").length > 0) {
                                        $("#bundle-option-" + valueItem.option_id + "-qty-input")[0].value = qty;
                                        $("#bundle-option-" + valueItem.option_id + "-qty-input")[0].setAttribute('disabled', true);
                                    }
                                }else{
                                    if ($("#bundle-option-" + valueItem.option_id + "-qty-input").length > 0) {
                                        $("#bundle-option-" + valueItem.option_id + "-qty-input")[0].removeAttribute('disabled');
                                    }
                                }
                                ...
                                price = parseFloat(price) + parseFloat(qty) * parseFloat(valueItem.selection_price_value);
                            }
                        });
