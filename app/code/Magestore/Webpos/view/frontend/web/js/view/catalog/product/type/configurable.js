    ...
    function ($, ko, Component, _, mageTemplate, ProductFactory, priceHelper, swatchModel, alertHelper, $t, productModel) {
        return {
            ...
            _initializeOptions: function () {
                ...
                var swatchAllOption = [];
                var allOptionArray = Object.keys(allOption).map(function (key) { return allOption[key]; });
                ...
                deferred.done(function (data) {
                    if (data && typeof data.swatches != 'undefined') {
                        ...
                        $.each(optionValue, function (index, value) {
                            ...
                            if (swatchData.type == '2') {
                                self._pushColorUrl(swatchData.value, value.id, attributeValue.id);
                            }
                        });
            ...
            _pushColorUrl: function (color, optionValue, attributeValue) {
                var swatchOption = $('#swatch-option');
                var oldHtml = swatchOption.html();

                swatchOption.html(oldHtml + "<div id='" + optionValue + "' class='swatch-option attribute" + attributeValue + "' data-attribute= '" + attributeValue + "' style='background-size: cover;background: url("+ window.webposConfig.swatch_media_url + color + ")" +"'></div>");
            },
