...
class OrderRepository implements \Magestore\Webpos\Api\Sales\OrderRepositoryInterface
{
    ...
    public function getList(\Magento\Framework\Api\SearchCriteria $searchCriteria)
    {
        ...
        foreach ($searchResult->getItems() as $order) {
            try{
                $purchase = $rewardsPurchase->getByQuote($order->getQuoteId());
                if($purchase) {
                    if($purchase->getEarnPoints()) {
                        $order->setData("rewards_earn", $purchase->getEarnPoints());
                    }
                    if($purchase->getSpendPoints()) {
                        $order->setData("rewards_spent", $purchase->getSpendPoints());
                        $order->setData("rewards_base_discount", $order->getBaseDiscountAmount());
                        $order->setData("rewards_discount", $order->getDiscountAmount());
                    }
                }
                $this->setShippingAssignments($order);
            }catch(\Exception $e){
