...
class Order extends \Magestore\Webpos\Helper\Data
{
     ...
     */
    public function createShipmentAndInvoice($orderId, $order, $create_invoice, $create_shipment, $items_to_ship = [], $tracks = [], $flag =false) {
     ...
     * @return array
     */
    public function prepareItemShipment($order,$flag = false) {
        $data = [];
        foreach ($order->getAllItems() as $item) {
            $data[$item->getId()] = ($flag==true) ? $item->getQtyToShip() : ($item->getQty()? $item->getQty() : $item->getQtyOrdered());
        }
        return $data;
    }
