...
namespace Custom\SalesPlugin\Model\Ups;
...
class Carrier extends \Magento\Ups\Model\Carrier
{
    ...
    {   
        ...
        $ship_to_other_flag = 0;
        if ($request->getRecipientAddressCountryCode() != "CA") {
            $ship_to_other_flag = 1;
        }
        
        if($ship_to_other_flag){
        ...
        if ($request->getIsReturn()) {
            ...
            if($ship_to_other_flag){
                ...
                $shipperPart->addChild('AttentionName', '');
                $shipperPart->addChild('PhoneNumber', '');
            }
            //$shipperPart->addChild('AttentionName', $request->getShipperContactPersonName());
