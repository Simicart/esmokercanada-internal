<?php
/**
* Magedelight
* Copyright (C) 2017 Magedelight <info@magedelight.com>
*
* @category Magedelight
* @package Magedelight_Monerisca
* @copyright Copyright (c) 2017 Mage Delight (http://www.magedelight.com/)
* @license http://opensource.org/licenses/gpl-3.0.html GNU General Public License,version 3 (GPL-3.0)
* @author Magedelight <info@magedelight.com>
*/

namespace Magedelight\Monerisca\Controller\Adminhtml\Cards;

class Edit extends \Magedelight\Monerisca\Controller\Adminhtml\Cards\Monerisca
{
    public function execute()
    {
        $editId = $this->getRequest()->getParam('customercardid', null);
        if ($editId!==null) {
            $cardModel = $this->cardsFactory->create();
            $cardModel->getResource()->load($cardModel, $editId);
            $cardData = $cardModel->getData();
        }
        $resultLayout = $this->resultLayoutFactory->create();
        $resultLayout
            ->getLayout()
            ->getBlock('md.moneris.ajax.form')->setCard($this->_jsonEncoder->encode($cardData));
        return $resultLayout;
    }
}
