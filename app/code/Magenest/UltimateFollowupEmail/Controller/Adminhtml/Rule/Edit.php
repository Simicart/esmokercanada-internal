...
class Edit extends Rule
{
    ...
    public function execute()
    {
        ...
        try {
            ...
            $saleRuleModel->setData('conditions_serialized', json_encode(unserialize($model->getData('conditions_serialized'))));
        }
        $catalogRule->setData('conditions_serialized', $model->getData('conditions_serialized'));
        try {
            $catalogRule->getConditions();
        } catch (\InvalidArgumentException $e) {
            $catalogRule->setData('conditions_serialized', json_encode(unserialize($model->getData('conditions_serialized'))));
        }
